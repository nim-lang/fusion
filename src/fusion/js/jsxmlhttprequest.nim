...
when defined(js):
  ...
  func setRequestHeader*(this: XMLHttpRequest; keyValuePairs: openArray[array[2, cstring]]) {.
  importjs: "(() => { const rqst = #; #.forEach((item) => rqst.setRequestHeader(item[0], item[1])) })()".}
    ## Same as `setRequestHeader` but takes `openArray[array[2, cstring]]`.


  runnableExamples:
    when defined(nimJsXmlhttprequestTests):
